...
import { vtsAdminLogin } from "@/app/actions/vts-auth";

export default function VtsAdminLoginPage() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await vtsAdminLogin({ username: "ADMIN", password });
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-amber-50 px-4">
      <div className="w-full max-w-sm bg-white rounded-lg shadow p-6">
        <h1 className="text-xl font-semibold text-amber-900 mb-1">Admin Login</h1>
