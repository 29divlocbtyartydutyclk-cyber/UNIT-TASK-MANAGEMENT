...
export default function CourseAdminLoginPage() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await courseLogin({ serviceNumber: "ADMIN", password });
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-study-50 px-4">
      <div className="w-full max-w-sm bg-white rounded-lg shadow p-6">
        <h1 className="text-xl font-semibold text-study-800 mb-1">Admin Login</h1>
        <p className="text-sm text-parchment-600 mb-6">
          Username is fixed as <span className="font-mono">ADMIN</span>.
        </p>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <p className="text-center text-sm">
            <Link href="/course" className="text-parchment-500">
              &larr; Back
            </Link>
          </p>
        </form>
